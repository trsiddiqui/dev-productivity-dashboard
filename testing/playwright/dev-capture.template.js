async (page) => {
  const outDir = '__OUT_DIR__';
  const baseUrl = '__BASE_URL__';

  function formatOrdinal(day) {
    const remainder = day % 10;
    const teen = day % 100;
    if (teen >= 11 && teen <= 13) return `${day}th`;
    if (remainder === 1) return `${day}st`;
    if (remainder === 2) return `${day}nd`;
    if (remainder === 3) return `${day}rd`;
    return `${day}th`;
  }

  function formatDayAriaLabel(year, monthIndex, day) {
    const date = new Date(Date.UTC(year, monthIndex, day));
    const weekday = date.toLocaleString('en-US', { weekday: 'long', timeZone: 'UTC' });
    const month = date.toLocaleString('en-US', { month: 'long', timeZone: 'UTC' });
    return `${weekday}, ${month} ${formatOrdinal(day)}, ${year}`;
  }

  function escapeRegex(value) {
    return value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  }

  async function chooseDeveloper(login) {
    const input = page.getByPlaceholder('Search GitHub users...').first();
    await input.click();
    await input.fill('');
    await input.fill(login);
    const option = page.getByRole('option', {
      name: new RegExp(escapeRegex(login), 'i'),
    }).first();
    const optionVisible = await option.isVisible().catch(() => false);
    if (optionVisible) {
      await option.click();
    } else {
      await input.press('Tab');
    }
  }

  async function setDateRange(rangeIndex, range) {
    const trigger = page.locator('button[aria-haspopup="dialog"]').nth(rangeIndex + 1);
    await trigger.click();

    const dialog = page.getByRole('dialog', { name: 'Choose date range' });
    await dialog.waitFor({ state: 'visible', timeout: 10000 });
    const captions = dialog.locator('.rdp-caption_label');

    for (let i = 0; i < 24; i += 1) {
      const visibleCaptions = (await captions.allTextContents()).map((text) => text.trim()).filter(Boolean);
      if (visibleCaptions.includes(range.monthLabel)) break;
      const currentDate = new Date(`${visibleCaptions[0]} 1 UTC`);
      const targetDate = new Date(`${range.monthLabel} 1 UTC`);
      if (currentDate > targetDate) {
        await dialog.locator('.rdp-button_previous').click();
      } else {
        await dialog.locator('.rdp-button_next').click();
      }
      await page.waitForTimeout(120);
    }

    const [yearText, monthText] = range.label.split('-');
    const year = Number(yearText);
    const monthIndex = Number(monthText) - 1;

    for (const targetDay of [1, range.lastDay]) {
      const label = formatDayAriaLabel(year, monthIndex, targetDay);
      const dayButton = dialog.getByLabel(label).last();
      await dayButton.waitFor({ state: 'visible', timeout: 10000 });
      await dayButton.click();
      await page.waitForTimeout(150);
    }

    await dialog.waitFor({ state: 'hidden', timeout: 10000 });
  }

  const username = '__USERNAME__';
  const password = '__PASSWORD__';
  const settings = {
    githubToken: '__GITHUB_TOKEN__',
    githubOrg: '__GITHUB_ORG__',
    jiraBaseUrl: '__JIRA_BASE_URL__',
    jiraEmail: '__JIRA_EMAIL__',
    jiraToken: '__JIRA_API_TOKEN__',
    jiraStoryPointsField: '__JIRA_STORY_POINTS_FIELD__',
    testRailBaseUrl: '__TESTRAIL_BASE_URL__',
    testRailEmail: '__TESTRAIL_EMAIL__',
    testRailToken: '__TESTRAIL_API_TOKEN__',
  };
  const stored = { username, ...settings };
  const cookieValue = encodeURIComponent(JSON.stringify(stored));

  const developers = __DEVELOPERS_JSON__;
  const monthPairs = __MONTH_PAIRS_JSON__;

  await page.setViewportSize({ width: 1600, height: 1400 });

  await page.goto(`${baseUrl}/`, { waitUntil: 'networkidle' });
  const signInButton = page.getByRole('button', { name: 'Sign in' });
  if (await signInButton.isVisible().catch(() => false)) {
    await page.getByLabel('Username').fill(username);
    await page.getByLabel('Password').fill(password);
    await Promise.all([
      page.waitForURL(/\/settings|\/individual|\/qa|\/contributions/, { timeout: 30000 }),
      signInButton.click(),
    ]);
  }

  await page.evaluate(({ username: activeUser, settings: runtimeSettings, cookie }) => {
    window.localStorage.setItem(`dpd-runtime-settings:${activeUser}`, JSON.stringify(runtimeSettings));
    document.cookie = `dpd_runtime_settings=${cookie}; Path=/; Max-Age=31536000; SameSite=Lax`;
  }, { username, settings, cookie: cookieValue });

  await page.goto(`${baseUrl}/contributions`, { waitUntil: 'networkidle' });
  await page.keyboard.press('Meta+0').catch(() => {});
  await page.getByText('Individual Contribution Dashboard').waitFor({ state: 'visible', timeout: 30000 });
  await page.getByRole('button', { name: 'Date Comparison' }).click();
  await page.waitForTimeout(500);

  const screenshots = [];

  for (const developer of developers) {
    await chooseDeveloper(developer.login);

    for (const [leftRange, rightRange] of monthPairs) {
      await setDateRange(0, leftRange);
      await setDateRange(1, rightRange);

      await Promise.all([
        page.waitForResponse(
          (resp) => resp.url().includes('/api/contributions') && resp.ok(),
          { timeout: 300000 },
        ),
        page.waitForResponse(
          (resp) => resp.url().includes('/api/contributions') && resp.ok(),
          { timeout: 300000 },
        ),
        page.getByRole('button', { name: 'Fetch comparison' }).click(),
      ]);

      await page.waitForTimeout(2500);

      const fileName = `${developer.login}-${leftRange.label}_vs_${rightRange.label}.png`;
      await page.screenshot({
        path: `${outDir}/${fileName}`,
        fullPage: true,
      });
      screenshots.push({
        fileName,
        title: `${developer.name} · ${leftRange.label} vs ${rightRange.label}`,
      });
    }
  }

  return { outDir, screenshots };
}
